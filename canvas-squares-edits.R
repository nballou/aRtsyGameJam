...
canvas_squares <- function(colors, background = "#080808", cuts = 5, ratio = 1.618,
                           resolution = 2000, noise = FALSE) {
  if (cuts <= 1) {
  ...
  my.colour1 <- toString(readline(prompt = "Enter your first colour "))
  my.colour2 <- toString(readline(prompt = "Enter your second colour "))
  artwork <- ggplot2::ggplot(data = full_canvas, ggplot2::aes(x = x, y = y, fill = z)) +
    ggplot2::geom_raster(interpolate = FALSE, alpha = 1) +
    ggplot2::xlim(c(0, resolution + 3)) +
    ggplot2::ylim(c(0, resolution + 1)) +
    ggplot2::scale_fill_gradient(low = toString(my.colour1), high = toString(my.colour2))
