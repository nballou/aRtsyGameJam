.myCreateBasePolygon <- function(color, nlayers, corners, resolution, mean = userMean) {
  if (nlayers == 1) {
    xmid <- (resolution / 2)
    ymid <- (resolution / 2)
  } else {
  ...
  varsegments <- stats::rnorm(nrow(coords), mean = mean, sd = 1.5)
  canvas <- data.frame(x = coords$x, y = coords$y, s = varsegments)
  canvas <- aRtsy:::deform(canvas, maxdepth = 5, resolution)
  return(canvas)
}
...
my_canvas_watercolors <- function(colors, background = "#fafafa", layers = 50,
  ...
  basePolygons <- list()
  for (i in 1:nlayers) {
    basePolygons[[i]] <- .myCreateBasePolygon(i, nlayers, corners[i], resolution, mean = userMean)
  }
  for (i in 1:length(colorSequence)) {
    canvas <- basePolygons[[labelSequence[i]]]
    canvas <- aRtsy:::deform(canvas, maxdepth = depth, resolution)
